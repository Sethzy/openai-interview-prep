...
def _find_chrome_cookie_dbs() -> list[Path]:
    """Find Chrome Cookies databases on macOS.

    Chrome stores cookies per profile. The old single-Default lookup misses
    users who are logged in under a named profile, so try Default first and
    then scan recently-used Profile directories.
    """
    candidates: list[Path] = []
    configured_profile = os.environ.get("LAST30DAYS_CHROME_PROFILE", "").strip()
    if configured_profile:
        configured = CHROME_BASE_DIR / configured_profile / "Cookies"
        return [configured] if configured.exists() else []

    if CHROME_COOKIES_DB.exists():
        candidates.append(CHROME_COOKIES_DB)
    ...
    try:
        ...
        for child in sorted(profile_dirs, key=lambda p: p.stat().st_mtime, reverse=True):
...
def extract_chrome_cookies_macos(domain: str, cookie_names: list[str]) -> Optional[dict[str, str]]:
    ...
    for db_path in _find_chrome_cookie_dbs():
        cookies = _extract_chromium_cookies_macos(
